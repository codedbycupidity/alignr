...
import { Block, PollContent } from '../types/block';
...
export default function VotingBlock({
  ...
  const pollContent = block.content as PollContent;
  const [newOption, setNewOption] = useState('');
  const [voterName, setVoterName] = useState('');

  const updateContent = useCallback((updatedContent: Partial<PollContent>) => {
    if (!onChange) return;

    const newContent: PollContent = {
      ...pollContent,
      ...updatedContent,
      type: 'poll',
      totalVotes: (updatedContent.options || pollContent.options).reduce((sum, opt) => sum + opt.votes, 0)
    };
  ...
  const handleVote = (optionId: string) => {
    ...
    const updated = pollContent.options.map(opt => {
      if (opt.id === optionId) {
        ...
        if (!pollContent.allowMultipleVotes) {
          // Remove vote from other options if multiple votes aren't allowed
